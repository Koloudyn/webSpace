...
const images = document.querySelectorAll(".image-with-text > img");
const l = images.length;

for(var i = 0; i < l; i++) {
  images[i].addEventListener("click", function(i) {
    var currentImg = this;
    const parentItem = currentImg.parentElement.parentElement, screenItem = document.createElement('div');
    ...
    if (l > 1) {
      if (route == first) {
        prevBtn.hidden = true;
        var prevImg = false;
        var nextImg = currentImg.parentElement.parentElement.nextElementSibling.firstElementChild.firstElementChild;
      }
      else if (route == last) {
        nextBtn.hidden = true;
        var nextImg = false;
        var prevImg = currentImg.parentElement.parentElement.previousElementSibling.firstElementChild.firstElementChild;
      }
      else {
        var prevImg = currentImg.parentElement.parentElement.previousElementSibling.firstElementChild.firstElementChild;
        var nextImg = currentImg.parentElement.parentElement.nextElementSibling.firstElementChild.firstElementChild;
    ...
    function prev() {
      prevImg = currentImg.parentElement.parentElement.previousElementSibling.firstElementChild.firstElementChild;
      imgItem.innerHTML = '<img src="' + prevImg.src + '">';
      currentImg = currentImg.parentElement.parentElement.previousElementSibling.firstElementChild.firstElementChild;
    ...
    function next() {
      nextImg = currentImg.parentElement.parentElement.nextElementSibling.firstElementChild.firstElementChild;
      imgItem.innerHTML = '<img src="' + nextImg.src + '">';
      currentImg = currentImg.parentElement.parentElement.nextElementSibling.firstElementChild.firstElementChild;
      var mainImg = document.querySelector(".gg-image > img").src;
